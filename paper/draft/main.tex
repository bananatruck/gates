...
In the scaffold we audited, the cause was mechanical: the host cut experiment output to 1,000 characters before any agent read it, so the writer never saw the real numbers and the failure detector never saw the crash.
...
In the Agent Laboratory scaffold we audited, the mechanism was plain.
The host cut experiment output to 1,000 characters before the writing agent read it, and it appended the crash marker \emph{after} the program's own output, so the same cut dropped the marker.
A run that raised \texttt{NameError} on every attempt scored 1.0 from the reward model and went into the paper with a full results table.
The model did not need to be dishonest; nothing ever showed it the numbers.
...
BadScientist~\cite{jiang2025badscientist} shows that LLM reviewers accept fabricated papers with no experiments at high rates under presentation-level manipulation.
...
The policies on a spent budget differ on purpose (Table~\ref{tab:exhaustion}): Gate 2 must not block a genuinely new result forever, while an unverifiable experiment or manuscript must not be emitted.
...
The model under test is DeepSeek V4.1 Flash; DeepSeek V4 Pro repeats MLR-Bench at levels 0 and 3, to test whether the effect belongs to the information flow or to one model.
The faked-result labels and the MLR-Judge scores come from Gemini Pro and Claude Sonnet, averaged as MLR-Judge does, so no judge is the model under test.
Every run records its cost and wallclock, and we report both.

\paragraph{Significance, fixed before the pilot.}
Level 0 and level 3 run on the same task with the same seed, so each task-seed is a pair.
Per benchmark, an exact McNemar test on the discordant pairs tests the integrity event, with Holm's correction across the three benchmarks at $\alpha = 0.05$.
The task score must be non-inferior: the lower bound of a seeded 95\% paired-bootstrap interval of level 3 minus level 0 stays above $-0.5$ MLR-Judge points, $-5$ points of pass@1, and $-10$ points of honest admission.
The pilot's discordance sets the seed count for 80\% power.
...
The Gate 1 campaign ran on DeepSeek V4 Flash and the benchmark runs on V4.1 Flash, which DeepSeek now serves under the old name too, so the campaign is context for level 1, not a replication of it.
Gate 3's citation checks see arXiv identifiers only; a paper that cites none is not checkable, and we count it as such rather than as clean.
...
These results measure how the gates behave, and the repository already holds them (Figure~\ref{fig:mechanism}).
Gate 3 also writes each rendered claim's provenance chain, from task to command to log to value to claim, and reports the share that is unbroken.
...
\begin{figure*}[t]
\includegraphics[width=\textwidth]{../figures/fig8_mechanism.png}
\caption{Mechanism evidence, measured: what each gate catches and what it wrongly flags, with Wilson 95\% intervals.}
\label{fig:mechanism}
\end{figure*}
